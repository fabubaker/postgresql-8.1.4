...
_bt_prefixKeyCompress(Relation rel, BTItem lowItem, BTItem highItem)
{
  ...
  if(ATTR_IS_TEXT(attr)) {
    ...
    int compressedLen = 0; // Length of compressed key
    int lowTextLen = lowText->vl_len - 4;
    int highTextLen = highText->vl_len - 4;
    int minIterations = (lowTextLen > highTextLen) ? highTextLen : lowTextLen;
    ...
    // At best, compressedLen is equal to highTextLen. This happens
    // when low key and high key are the same. Only add one more character
    // for differentiation when this is NOT the case.
    if (compressedLen < highTextLen) {
        compressedLen++;
    }

      highText->vl_len = compressedLen + 4; // Account for the 4 bytes for vl_len itself.
      toReturn = highTextLen - compressedLen;
    /*
	  ...
	  bool newitemonleft)
{
  ...
  rbuf = _bt_getbuf(rel, P_NEW, BT_WRITE); // Creates a new buffer for right child
